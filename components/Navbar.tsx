            ...
            <div className="px-4 md:px-6 py-3 flex items-center justify-between">
                ...
                <div className="hidden md:flex items-center gap-3 lg:gap-4">
                    {user ? (
                        ...
                        <>
                            {/* Guest Badge */}
                            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl border ${quota && quota.remaining > 0
                                ? 'bg-caution-amber/10 border-caution-amber/20'
                                : 'bg-objection-red/10 border-objection-red/20'
                                }`}>
                                <span className={`text-xs font-medium ${quota && quota.remaining > 0
                                    ? 'text-caution-amber'
                                    : 'text-objection-red'
                                    }`}>
                                    {quota && quota.remaining > 0 ? '🎁 1 free verdict' : '0 verdicts left'}
                                </span>
                            </div>
            ...
            <div className={`md:hidden overflow-hidden transition-all duration-300 ease-out ${isMenuOpen ? 'max-h-64 opacity-100' : 'max-h-0 opacity-0'}`}>
                <div className="px-4 pb-4 pt-2 border-t border-white/5">
                    {user ? (
                        ...
                        <div className="space-y-3">
                            <div className={`flex items-center justify-center gap-2 p-3 rounded-xl border ${quota && quota.remaining > 0 ? 'bg-caution-amber/10 border-caution-amber/20' : 'bg-objection-red/10 border-objection-red/20'
                                }`}>
                                <span className="text-2xl">{quota && quota.remaining > 0 ? '🎁' : '🚫'}</span>
                                <span className={`text-sm font-medium ${quota && quota.remaining > 0 ? 'text-caution-amber' : 'text-objection-red'}`}>
                                    {quota && quota.remaining > 0 ? 'Try 1 verdict free!' : 'Sign in for more'}
                                </span>
                            </div>
                            <div className="flex gap-2">
